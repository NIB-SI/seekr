...
Note: If R is already started in an existing assay, the following chunk is not needed, set \Sexpr{eval} to FALSE.
...
uploadRoot
recursive <- TRUE
forUpload <- skFilesToUpload( root = uploadRoot
    , skignore = seekignore
    , recursive = recursive
    )
```
...
```{r}
if(length(forUpload) < 15) forUpload else {
head(forUpload)
tail(forUpload)
}
```
...
```{r upload_files}
ind <- c(1:5,sample(1:length(forUpload),100))
ind <- 1:8
head(forUpload[ind])
ind <- 1:length(forUpload)
#
#skUploadFiles(forUpload, test=TRUE, append=FALSE)
skUploadFiles(forUpload[ind], test=TRUE, append=FALSE, verbose=TRUE, invname="_I_01_LabTrials-test-private4")
```

# After upload

You can follow the progress of the upload in console. In addition, you get the upload details in two files: the log-file (FAIRDOM.log) and and file with the name that starts with 'Upload ...' and date-time (e.g. Upload-2021-11-07_16_20_23.txt). This file contains uploaded file names and links to uploaded files on FAIRBOMHub. Both files appear in the working directory.
