...
More elgant way might be a call to skRelationships.

```{r}
skRelationships("proj",pid,"program")
```
...
If the layer (investigation) does not exist, the function skFindId returns zero in the place of 'id'. There is a convenience functio \fct{skExists} that checks existence of a layer of object.

```{r}
iid
pini$I$name
skExists("inv",pini$I$name, verbose=TRUE)
```
...
```{r eval=(iid == 0)}
if( !skExists("inv",pini$I$name)) skCreate("investigations", meta=.imeta)
...
```{r}
r <- skRead("investigations", options("sk.iid"))
r <- skContent(r)
names(r$attributes)
...
If you wants to be sure, you can update the seekr  seekr options.
