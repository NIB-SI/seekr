...
<<>>=
skFilesToUpload <- function(root=.iroot, na.exclude=FALSE, md.exclude=TRUE, verbose=TRUE, quiet=FALSE, skignore=file.path(root,"seekignore.txt")){
...
seekignore <- c(seekignore,skDoNotUpload(root,na.exclude, verbose, quiet))
if(md.exclude) seekignore <- c(seekignore,skIgnoreBlankReadme(root, verbose, quiet))
...
dirs <-  dir(root
             ...
             , recursive=TRUE
             , include.dirs=FALSE)
dirs <- list.files(root, full.names=TRUE, recursive=TRUE)
...
skFilesToUpload()
skFilesToUpload(md.exclude=FALSE)
...
<<>>
skDoNotUpload <- function(root = .iroot ,na.exclude=FALSE, verbose=TRUE, quiet=FALSE){
#
mdir <- dir(root,pattern="*_METADATA.TXT", recursive=TRUE)
mlength <- length(mdir)
ignore <- NULL
for(fn in mdir) {
#fn <- mdir[2]
    ...
    if(  value %in% c("n", "N")  | (is.na(value)& na.exclude)  ) {
...
skDoNotUpload(.proot)
skDoNotUpload(.proot, quiet=TRUE)

@

<<>>
skIgnoreBlankReadme <- function(root = .iroot ,na.exclude=TRUE, quiet=FALSE){
#
mdir <- dir(root,pattern="README.MD", recursive=TRUE)
#print(mdir)
ignore <- NULL
for(fn in mdir) {
