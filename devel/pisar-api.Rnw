...
#' id
#' }
fhData <- function(r, type, ...){
  if(class(r)=="seek_api") r <- r$response
  jsn <- "application/json"
  if(missing(type)) invisible(content(r,"parsed",type=jsn)$data) else
  invisible(content(r,"parsed",type=jsn)$data[[type]])
}
fhDatas <- fhData
@
...
#' }
#'
#'  fhIni(prid = 26, pid=104, iid=fhData(si)$id, test=TRUE)
#'  options("fhub")
